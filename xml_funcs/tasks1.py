...
### begin of gfs --------------------------------------------------------
def gfs(xmlFile, expdir, meta_id, start, stop):
  meta_id=meta_id
  cycledefs=meta_id
  # metatask (nested or not)
  start=start
  stop=stop
  interval=1
  meta_hr= ''.join(f'{i:03d} ' for i in range(start,stop+1,interval)).strip()
  command=f'{expdir}/gfs.link_to_nco_namespace.sh'
  meta_bgn=f'''
<metatask name="{meta_id}">
<var name="fhr">{meta_hr}</var>'''
  meta_end=f'\
  ...
  <dependency>
  <and>{timedep}
    <datadep age="00:01:00"><cyclestr>{COMINgfs}/@y@j@H000#fhr#</cyclestr></datadep>
  </and>
  </dependency>'''
  #
  xml_task(xmlFile,expdir,task_id,cycledefs,dcTaskEnv,dependencies,True,meta_id,meta_bgn,meta_end,command)
